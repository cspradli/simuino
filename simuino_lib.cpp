...
//====================================
{
  ...
  char str[80], str1[12], str2[12], *p;
  {
    if (strstr(event, "analogRead") || strstr(event, "digitalRead"))
    {
      ...
      g_pinNo = pin;
      g_pinValue = value;
      if (strstr(event, "analog"))
        g_pinType = ANA;
      if (strstr(event, "digital"))
        g_pinType = DIG;
      return (g_pinType);
    }
    return (0);
...
//====================================
{
  ...
  if (win == uno)
  {
    ...
    wmove(win, 2, 2);
    if (g_currentSketchStatus == SO_VOID)
      wprintw(uno, "Status: -            ");
    if (g_currentSketchStatus == SO_SELECTED)
      wprintw(uno, "Status: selected     ");
    if (g_currentSketchStatus == SO_LOADED)
      wprintw(uno, "Status: loaded       ");
    if (g_currentSketchStatus == SO_RUN_ERROR)
      wprintw(uno, "Status: runtime error");
    if (g_currentSketchStatus == SO_COMP_ERROR)
      wprintw(uno, "Status: compile error");
  }
  if (win == ser)
  {
    wmove(win, 0, 2);
...
//====================================
{
  ...
  else
  {
    ...
    {
      wmove(msg, 1, 1);
      wprintw(msg, "No scenario data in sketch");
    }
    show(msg);
    fclose(in);
  }
...
//====================================
{
  ...
  wmove(uno, dp - 1, RF);
  waddch(uno, ACS_ULCORNER);
  wmove(uno, dp - 1, RF + board_w - 3);
  waddch(uno, ACS_URCORNER);
  wmove(uno, ap + 1, RF);
...
//====================================
{
  ...
  while (fgets(row, SIZE_ROW, in) != NULL)
  {
    ...
    if (strstr(row, "setup(") != NULL)
      g_row_setup = count;
    if (strstr(row, "loop(") != NULL)
      g_row_loop = count;
...
//====================================
{
  ...
  sprintf(syscom, "cd servuino; g++ -O2 -o servuino servuino.cpp > g++.result 2>&1;");
  x = system(syscom);
