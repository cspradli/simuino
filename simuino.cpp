...
//====================================
{
	...
	if (res == 0)
	{
		if (confSteps < 0)
			confSteps = 100;
		if (confSteps > MAX_STEP)
			confSteps = MAX_STEP - 1;
...
//====================================
{
	...
	while (strstr(str, "ex") == NULL)
	{
		...
		wprintw(uno, "                                                  ");
		if (g_silent == S_NO)
			mvwprintw(uno, board_h - 2, 1, "C%1d>", confWinMode);
		if (g_silent == S_YES)
			mvwprintw(uno, board_h - 2, 1, "C%1d<", confWinMode);
		...
		else if (strstr(sstr, "rem")) //
		{
			if (n == 4)
			{
				if (strstr(command[1], "a"))
					g_pinType = ANA;
				if (strstr(command[1], "d"))
					g_pinType = DIG;
